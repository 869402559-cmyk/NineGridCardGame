...
- **当前项目状态**：核心战斗（5v5 九宫格、兵种动作、士气大招、兵种克制、医师治疗/大招、双层计算公式、SSR华佗抽卡、布阵、多账号存档、洗练洗等级、批量解雇）已完备可运行。
...
│   ├── GameData.gd             # 单例组件：CSV解析、属性计算、账号/存档、解雇与洗练逻辑
│   ├── LoginUI.gd              # 登录与注册界面
...
1. **账号隔离与初始资金**：账号密码与英雄存档统一存储于 `user://accounts.json`。管理员 `admin` 登录默认提供 **1 亿金币 & 1 亿经验** 方便测试。
2. **战斗时存档锁定**：战斗过程中主界面“💾 保存存档”按钮被锁定（提示 `⚠️ 战斗中不可保存`）。
3. **单次战斗奖励原子性**：`BattleUI.gd` 中设置独占标记 `is_reward_given`，防止双重结算。
