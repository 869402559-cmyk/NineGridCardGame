...
- **当前项目状态**：核心战斗（5v5 九宫格、兵种动作、士气大招、兵种克制、抽卡、布阵、多账号存档）已完备可运行。
...
## ⚡ 3. 核心游戏机制与数值公式 (Core Game Mechanics)

### 3.1 英雄与兵种解耦（7:3 复合属性）
- **属性叠加公式**：
  $$	ext{最终战斗属性} = 	ext{round}(	ext{英雄基础属性} 	imes 0.7 + 	ext{兵种基础属性} 	imes 0.3)$$
  - **参与 7:3 复合计算的属性**：普通攻击 (`atk`)、普通防御 (`def`)、战法攻击 (`satk`)、战法防御 (`sdef`)。
  - **直接继承/累加的属性**：
    - $	ext{最终 HP} = 	ext{英雄基础 HP}$（受到英雄等级成长提升）。
    - $	ext{最终 速度 Spd} = 	ext{英雄基础 Spd}$。
- **兵种专属闪避率 (MISS)**：
  - 闪避判定绑定在兵种上，受击方独立触发：
    - 弓兵 (Archer)：20%
    - 鼓手 (Drummer)：15%
    ...
    - 步兵 (Infantry)：5%
- **兵种克制与克制系数**：
  - 克制关系基于 `troops.csv` 中的 `target_type` 与 `bonus_rate` 计算：
    - 虎豹骑（骑兵）克制步兵（+10% 伤害）
    - 重装步兵（步兵）克制弓兵（+10% 伤害）
    - 白马义从（弓兵）克制骑兵（+10% 伤害）
  - 克制生效时在战斗日志中明确提示，且加成系数参与最终伤害结算。

### 3.2 3x3 九宫格布阵 (3x3 Formation)
...
### 3.3 5v5 回合制战斗与动作动画 (Battle System)
- **行动顺序与轮次**：
  - 根据双方存活单位的速度（`Spd`）降序排列决定行动次序。
  - 相同速度时玩家单位优先行动。
- **寻敌与攻击规则**：
