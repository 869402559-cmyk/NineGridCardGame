...
- **当前项目状态**：核心战斗（5v5 九宫格、兵种动作、士气大招、兵种克制、医师治疗/大招、双层计算公式、SSR华佗抽卡、布阵、多账号存档）已完备可运行。
...
为了保证游戏数值的清晰性与扩展性，本项目采用**“面板属性复合”与“战斗动态伤害/治疗”解耦的两阶段数值结算模型**。任何接手本项目的 AI 或开发者请严格遵循以下计算层次：
...
- **兵种专属动作与打击反馈**：
  ...
  - **医师 (专属治疗机制)**：不攻击敌方！普攻与大招高亮发绿光，为己方伤势最重（或全队）恢复兵力，弹出绿色回血飘字。
...
│   └── BattleUI.gd             # 战斗场景（回合循环、行动排序、寻敌/救护、动画 Tween、平滑日志滚动、原子化结算）
└── scenes/                     # 场景文件 (.tscn)
    ├── LoginUI.tscn
    ├── MainUI.tscn
    ├── FormationUI.tscn
...
| 字段 | 类型 | 说明 |
| :--- | :--- | :--- |
| `id` | String | 英雄唯一 ID（如 `h_16` 华佗） |
| `name` | String | 英雄名称（如 `华佗`） |
| `quality` | String | 品质等级（`UR`, `SSR`, `SR`, `R`, `N`） |
| `troop_id` | String | 默认绑定兵种 ID（如 `t_doctor` 医师） |
| `hp` | Int | 基础生命值 |
| `atk` | Int | 基础攻击力 |
| `def` | Int | 基础物理防御 |
| `satk` | Int | 基础战法攻击 |
| `sdef` | Int | 基础战法防御 |
| `spd` | Int | 基础速度 |
| `color` | String | 主主题十六进制颜色（如 `#FFAA00`） |

### 5.2 `troops.csv` (兵种配置表)
| 字段 | 类型 | 说明 |
| :--- | :--- | :--- |
| `id` | String | 兵种唯一 ID（如 `t_doctor`） |
| `name` | String | 兵种名称（如 `杏林医师`） |
| `type_name` | String | 兵种分类（`骑兵`, `步兵`, `弓兵`, `鼓手`, `战法`, `医师`） |
| `atk` | Int | 兵种基础攻击 |
| `def` | Int | 兵种基础防御 |
| `satk` | Int | 兵种基础战法攻击 |
| `sdef` | Int | 兵种基础战法防御 |
| `evade_rate` | Float | 兵种专属闪避率（如 `0.10` 代表 10%） |
| `target_type` | String | 克制目标兵种类型（医师无克制写 `无`） |
| `bonus_target` | String | 攻击范围分类（如 `友方治疗`） |
| `bonus_rate` | Float | 克制伤害加成比例（`0.0`） |
| `skill_name` | String | 战法技能名称（如 `悬壶济世`） |
| `skill_desc` | String | 技能描述 |
| `anim_type` | String | 攻击动画类型（`heal`） |
...
1. **技能数据驱动表 (`data/skills.csv`)**：
   - *现状*：技能效果（如鼓手加士气、医师全队加血、法师横排火攻）目前直接硬编码在 `BattleUI.gd` 中。
   - *规划*：新建 `skills.csv`，将技能伤害/治疗倍率、作用目标规则、士气消耗、动画标签数据驱动化。
